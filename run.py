...
def download_question(question, question_dir):
    ...
    if os.path.exists(os.path.join(question_dir, 'img')):
        shutil.rmtree(os.path.join(question_dir, 'img'))
    ...
    if not res_json['content']:
        ...
        content, meta = download_images(res_json['content'], meta)
...
def download_images(content, meta):
    ...
    for img in imgs:
        ...
        original_image = 'https://leetcode.com' + img if img.startswith('/') else img
        res = requests.get(original_image)
        imgname = meta['slug'] + '_' + str(counter) + ext
        with open(os.path.join(imgdir, imgname), 'wb') as f:
            f.write(res.content)
            sub[img] = '../img/%s' % imgname
            meta['images'][imgname] = original_image
        counter += 1
